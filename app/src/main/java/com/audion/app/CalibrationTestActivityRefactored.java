...
public class CalibrationTestActivityRefactored extends AppCompatActivity {
    ...
    private void showCalibrationSummary() {
        ...
        // Directly save calibration data without showing dialog
        saveCalibrationData();
    }
    ...
    private void navigateAfterCalibration() {
        // Simple logic: check which ear we just completed and navigate accordingly
        runOnUiThread(() -> {
            Intent intent;
            
            if ("RIGHT".equals(earSide)) {
                // Just finished RIGHT ear, now do LEFT ear
                Log.d("FlowDebug", "Finished RIGHT ear - navigating to LEFT ear calibration");
                ...
                // Just finished LEFT ear, check if RIGHT is already done
                dbExecutor.execute(() -> {
                    try {
                        AppDatabase db = AppDatabase.getInstance(this);
                        CalibrationProfileDao dao = db.calibrationProfileDao();
                        ...
                        runOnUiThread(() -> {
                            ...
                            if (rightProfile == null) {
                                // RIGHT not done yet, do it now
                                Log.d("FlowDebug", "Finished LEFT ear - navigating to RIGHT ear calibration");
                                nextIntent = new Intent(this, CalibrationInstructionActivity.class);
                                nextIntent.putExtra("EAR", "RIGHT");
                                nextIntent.putExtra("USER_ID", userId);
                                nextIntent.putExtra("HEARING_PROFILE_ID", hearingProfileId);
                                nextIntent.putExtra("FROM_NEW_PROFILE", fromNewProfile);
                            } else {
                                // Both ears done - go to completion
                                Log.d("FlowDebug", "Both ears calibrated - navigating to Test Completion");
                                nextIntent = new Intent(this, TestCompletionActivity.class);
                                nextIntent.putExtra("USER_ID", userId);
                                nextIntent.putExtra("HEARING_PROFILE_ID", hearingProfileId);
                                nextIntent.putExtra("FROM_NEW_PROFILE", fromNewProfile);
                            }
                        ...
                        Log.e(TAG, "Error checking RIGHT ear status: " + e.getMessage(), e);
                        runOnUiThread(() -> {
                            // Fallback - go to completion
                            Intent fallbackIntent = new Intent(this, TestCompletionActivity.class);
                            fallbackIntent.putExtra("USER_ID", userId);
                            fallbackIntent.putExtra("HEARING_PROFILE_ID", hearingProfileId);
                            fallbackIntent.putExtra("FROM_NEW_PROFILE", fromNewProfile);
                            startActivity(fallbackIntent);
                            finish();
                        });
                    }
                });
                return; // Early return since we're handling navigation in the db callback
            } else {
                // Unknown ear side, go to home
                Log.e("FlowDebug", "Unknown ear side: " + earSide);
