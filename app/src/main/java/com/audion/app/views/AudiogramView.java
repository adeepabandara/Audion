...
public class AudiogramView extends View {
    ...
    private void drawAxes(Canvas canvas) {
        ...
        // Y-axis title (outside, rotated)
        canvas.save();
        float yAxisTitleX = 40f; // Increased from 20 for more padding
        canvas.rotate(-90, yAxisTitleX, getHeight() / 2f);
        labelPaint.setTextAlign(Paint.Align.CENTER);
        labelPaint.setTextSize(28f);
        canvas.drawText("Hearing Level (dB HL)", yAxisTitleX, getHeight() / 2f, labelPaint);
        canvas.restore();
        ...
        // X-axis title (outside, below frequency labels)
        labelPaint.setTextAlign(Paint.Align.CENTER);
        labelPaint.setTextSize(28f);
        canvas.drawText("Frequency (Hz)", getWidth() / 2f, topMargin + chartHeight + 65, labelPaint);
    }
